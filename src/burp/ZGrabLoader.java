...
class ZgrabLoader {
    ...
    ZgrabLoader(Scan scanner) {
        ...
        try {
            ...
            //BulkUtilities.out(conn.createStatement().executeQuery("select * from requests").getString(1));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    void launchSmugglePipeline() {
        ...
        for (String domain: domains) {
            ...
            IHttpService service = BulkUtilities.callbacks.getHelpers().buildHttpService(domain, 443, true);
            scanner.doScan(request, service);
        }
        BulkUtilities.out("Scan complete");
    ...
    synchronized byte[] getResponse(String host, byte[] request) {
        try {
            ...
            query.setString(2, BulkUtilities.helpers.bytesToString(request));
            ResultSet res = query.executeQuery();

            if (res.isClosed()) {
                BulkUtilities.out("Couldn't find request");
                return null;
            }

            String resp = res.getString(2);
            if (resp == null) {
                BulkUtilities.out("returning timeout...");
                return "".getBytes();
            }
